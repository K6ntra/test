...
local stellarlib = {
  Themes = {
    Classic = {
      ...
      ["Stellar Icon"] = "rbxassetid://94923298565478"
    }
  },
  Info = {
    Version = "5.0"
...
local SetProps, SetChildren, InsertTheme, Create do
  InsertTheme = function(Instance, Type)
    table.insert(stellarlib.Instances, {
      Instance = Instance,
      Type = Type
    })
    return Instance
  ...
  local function Save(file)
    if readfile and isfile and isfile(file) then
      ...
      if type(decode) == "table" then
        if rawget(decode, "UISize") then stellarlib.Save["UISize"] = decode["UISize"] end
        if rawget(decode, "TabSize") then stellarlib.Save["TabSize"] = decode["TabSize"] end
        if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then stellarlib.Save["Theme"] = decode["Theme"] end
      end
    end
  end
  pcall(Save, "stellar lib.lua")
end

local ScreenGui = Create("ScreenGui", CoreGui, {
  Name = "stellar Library",
}, {
  Create("UIScale", {
    Scale = UIScale,
    Name = "Scale"
...
local function VerifyTheme(Theme)
  for name,_ in pairs(stellarlib.Themes) do
    if name == Theme then
      return true
    end
  end
...
local Theme = stellarlib.Themes[stellarlib.Save.Theme]

local function AddEle(Name, Func)
  stellarlib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
  local Element = stellarlib.Elements[Ele](Instance, props, ...)
  return Element
end
...
function stellarlib:SetTheme(NewTheme)
  if not VerifyTheme(NewTheme) then return end
  
  stellarlib.Save.Theme = NewTheme
  SaveJson("stellar lib.lua", stellarlib.Save)
  Theme = stellarlib.Themes[NewTheme]
  
  table.foreach(stellarlib.Instances, function(_,Val)
...
function stellarlib:SetScale(NewScale)
  NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
  UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function stellarlib:MakeWindow(Configs)
  local WTitle = Configs[1] or Configs.Name or Configs.Title or "stellar Library"
  ...
  local UISizeX, UISizeY = unpack(stellarlib.Save.UISize)
  ...
  local Title = InsertTheme(Create("TextLabel", TopBar, {
    Position = UDim2.new(0.007, 24, 0.54, 0),
    AnchorPoint = Vector2.new(0, 0.5),
    AutomaticSize = "XY",
    Text = WTitle,
    TextXAlignment = "Left",
  ...
  local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
    Size = UDim2.new(0, stellarlib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
...
function Window:MinimizeBtn()
  ...
  function Window:SelectTab(TabSelect)
    if type(TabSelect) == "number" then
      stellarlib.Tabs[TabSelect].func:Enable()
    else
      for _,Tab in pairs(stellarlib.Tabs) do
  ...
  function Window:MakeTab(paste, Configs)
    ...
    TIcon = stellarlib:GetIcon(TIcon)
...
Make("Corner", Selected, UDim.new(0.5, 0))
    ...
    local function Tabs()
      ...
      table.foreach(stellarlib.Tabs, function(_,Tab)
    ...
    FirstTab = true
    local Tab = {}
    table.insert(stellarlib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
    Tab.Cont = Container
    ...
    function Tab:AddSection(Configs)
      ...
      local Section = {}
      table.insert(stellarlib.Options, {type = "Section", Name = SectionName, func = Section})
    ...
    function Tab:AddButton(Configs)
      ...
      local Button = {}
      table.insert(stellarlib.Options, {type = "Button", Name = BName, func = Button})
  ...
  toggleGradient.Parent = Toggle
      ...
      local Toggle = {}
      table.insert(stellarlib.Options, {type = "Toggle", Name = TName, func = Toggle})
...
InsertTheme(SelectedFrame, "Theme")
      ...
      local Dropdown = {}
      table.insert(stellarlib.Options, {type = "Dropdown", Name = DName, func = Dropdown})
...
gradient.Parent = Indicator
      ...
      SliderIcon:GetPropertyChangedSignal("Position"):Connect(function()
        ...
        SaveSlider()
			...
			local Slider = {}
			table.insert(stellarlib.Options, {type = "Slider", Name = SName, func = Slider})
