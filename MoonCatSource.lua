...
local mooncatlib = {
  Themes = {
    Classic = {
      ...
      ["MoonCat Icon"] = "rbxassetid://139789411026767"
    }
  },
  Info = {
    Version = "5.0"
...
local SetProps, SetChildren, InsertTheme, Create do
  InsertTheme = function(Instance, Type)
    table.insert(mooncatlib.Instances, {
      Instance = Instance,
      Type = Type
    })
    return Instance
  ...
  local function Save(file)
    if readfile and isfile and isfile(file) then
      ...
      if type(decode) == "table" then
        if rawget(decode, "UISize") then mooncatlib.Save["UISize"] = decode["UISize"] end
        if rawget(decode, "TabSize") then mooncatlib.Save["TabSize"] = decode["TabSize"] end
        if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then mooncatlib.Save["Theme"] = decode["Theme"] end
      end
    end
  end
  pcall(Save, "MoonCat lib.lua")
end

local ScreenGui = Create("ScreenGui", CoreGui, {
  Name = "MoonCat Library",
}, {
  Create("UIScale", {
    Scale = UIScale,
    Name = "Scale"
...
local function VerifyTheme(Theme)
  for name,_ in pairs(mooncatlib.Themes) do
    if name == Theme then
      return true
    end
  end
...
local Theme = mooncatlib.Themes[mooncatlib.Save.Theme]

local function AddEle(Name, Func)
  mooncatlib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
  local Element = mooncatlib.Elements[Ele](Instance, props, ...)
  return Element
end
...
function mooncatlib:SetTheme(NewTheme)
  if not VerifyTheme(NewTheme) then return end
  
  mooncatlib.Save.Theme = NewTheme
  SaveJson("MoonCat lib.lua", mooncatlib.Save)
  Theme = mooncatlib.Themes[NewTheme]
  
  table.foreach(mooncatlib.Instances, function(_,Val)
...
function mooncatlib:SetScale(NewScale)
  NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
  UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function mooncatlib:MakeWindow(Configs)
  local WTitle = Configs[1] or Configs.Name or Configs.Title or "MoonCat Library"
  ...
  local UISizeX, UISizeY = unpack(mooncatlib.Save.UISize)
  ...
  local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
    Size = UDim2.new(0, mooncatlib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
...
function Window:MinimizeBtn()
  ...
  function Window:SelectTab(TabSelect)
    if type(TabSelect) == "number" then
      mooncatlib.Tabs[TabSelect].func:Enable()
    else
      for _,Tab in pairs(mooncatlib.Tabs) do
  ...
  function Window:MakeTab(paste, Configs)
    ...
    TIcon = mooncatlib:GetIcon(TIcon)
...
Make("Corner", Selected, UDim.new(0.5, 0))
    ...
    local function Tabs()
      ...
      table.foreach(mooncatlib.Tabs, function(_,Tab)
    ...
    FirstTab = true
    local Tab = {}
    table.insert(mooncatlib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
    Tab.Cont = Container
    ...
    function Tab:AddSection(Configs)
      ...
      local Section = {}
      table.insert(mooncatlib.Options, {type = "Section", Name = SectionName, func = Section})
    ...
    function Tab:AddButton(Configs)
      ...
      local Button = {}
      table.insert(mooncatlib.Options, {type = "Button", Name = BName, func = Button})
  ...
  toggleGradient.Parent = Toggle
      ...
      local Toggle = {}
      table.insert(mooncatlib.Options, {type = "Toggle", Name = TName, func = Toggle})
...
InsertTheme(SelectedFrame, "Theme")
      ...
      local Dropdown = {}
      table.insert(mooncatlib.Options, {type = "Dropdown", Name = DName, func = Dropdown})
...
gradient.Parent = Indicator
      ...
      SliderIcon:GetPropertyChangedSignal("Position"):Connect(function()
        ...
        SaveSlider()
			...
			local Slider = {}
			table.insert(mooncatlib.Options, {type = "Slider", Name = SName, func = Slider})
