...
local featherlib = {
  Themes = {
    Warm = {
      ...
      ["Feather Icon"] = "rbxassetid://94923298565478"
    }
  },
  Info = {
    Version = "4.1"
...
local SetProps, SetChildren, InsertTheme, Create do
  InsertTheme = function(Instance, Type)
    table.insert(featherlib.Instances, {
      Instance = Instance,
      Type = Type
    })
    return Instance
  ...
  local function Save(file)
    if readfile and isfile and isfile(file) then
      ...
      if type(decode) == "table" then
        if rawget(decode, "UISize") then featherlib.Save["UISize"] = decode["UISize"] end
        if rawget(decode, "TabSize") then featherlib.Save["TabSize"] = decode["TabSize"] end
        if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then featherlib.Save["Theme"] = decode["Theme"] end
      end
    end
  end
  pcall(Save, "feather lib.lua")
end
...
local function VerifyTheme(Theme)
  for name,_ in pairs(featherlib.Themes) do
    if name == Theme then
      return true
    end
  end
...
local Theme = featherlib.Themes[featherlib.Save.Theme]

local function AddEle(Name, Func)
  featherlib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
  local Element = featherlib.Elements[Ele](Instance, props, ...)
  return Element
end
...
function featherlib:SetTheme(NewTheme)
  if not VerifyTheme(NewTheme) then return end
  
  featherlib.Save.Theme = NewTheme
  SaveJson("feather lib.lua", featherlib.Save)
  Theme = featherlib.Themes[NewTheme]
  
  table.foreach(featherlib.Instances, function(_,Val)
...
function featherlib:SetScale(NewScale)
  NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
  UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function featherlib:MakeWindow(Configs)
  local WTitle = Configs[1] or Configs.Name or Configs.Title or "feather Library"
  local WMiniText = Configs[2] or Configs.SubTitle or "print? kk"
  local SaveCfg = Configs[3] or Configs.SaveFolder or false
  local SaveRejoin = Configs[4] or Configs.SaveRejoin or false
  local Flags = featherlib.Flags
  ...
  local UISizeX, UISizeY = unpack(featherlib.Save.UISize)
  ...
  local ImageLabel = Create("ImageLabel", TopBar, {
    Size = UDim2.new(0, 17, 0, 17),
    Position = UDim2.new(0, 5, 0.5, 0),
    AnchorPoint = Vector2.new(0, 0.5),
    BackgroundTransparency = 1,
    Image = Theme["Feather Icon"]
})
  ...
  local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
    Size = UDim2.new(0, featherlib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
...
function Window:CloseBtn()
    local Dialog = Window:Dialog({
      Title = "Close",
      Text = "Do you really want to close the Script?",
      Options = {
        {"Confirm", function()
          ScreenGui:Destroy()
        end},
...
function Window:MinimizeBtn()
  ...
  function Window:SelectTab(TabSelect)
    if type(TabSelect) == "number" then
      featherlib.Tabs[TabSelect].func:Enable()
    else
      for _,Tab in pairs(featherlib.Tabs) do
  ...
  function Window:MakeTab(paste, Configs)
    ...
    TIcon = featherlib:GetIcon(TIcon)
...
gradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(172, 163, 208)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(125, 86, 163))
})
...
Make("Corner", Selected, UDim.new(0.5, 0))
    ...
    local function Tabs()
      ...
      table.foreach(featherlib.Tabs, function(_,Tab)
    ...
    FirstTab = true
    local Tab = {}
    table.insert(featherlib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
    Tab.Cont = Container
    ...
    function Tab:AddSection(Configs)
      ...
      local Section = {}
      table.insert(featherlib.Options, {type = "Section", Name = SectionName, func = Section})
    ...
    function Tab:AddButton(Configs)
      ...
      local Button = {}
      table.insert(featherlib.Options, {type = "Button", Name = BName, func = Button})
    ...
    ColorSequenceKeypoint.new(0, Color3.fromRGB(172, 163, 208)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(86, 86, 163))})
  ...
  toggleGradient.Parent = Toggle
      ...
      local Toggle = {}
      table.insert(featherlib.Options, {type = "Toggle", Name = TName, func = Toggle})
    ...
    function Tab:AddDropdown(Configs)
      ...
      local function AddOption(OptionName)
        ...
        local SelectedFrame = Create("Frame", Option, {
    Position = UDim2.new(0, 1, 0.5),
...
gradient.Rotation = 90
gradient.Parent = SelectedFrame

Make("Corner", SelectedFrame, UDim.new(0.5, 0))

InsertTheme(SelectedFrame, "Theme")
      ...
      local Dropdown = {}
      table.insert(featherlib.Options, {type = "Dropdown", Name = DName, func = Dropdown})
...
gradient.Parent = Indicator
      ...
      SliderIcon:GetPropertyChangedSignal("Position"):Connect(function()
        ...
        SaveSlider()
			...
			local Slider = {}
			table.insert(featherlib.Options, {type = "Slider", Name = SName, func = Slider})
